...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive,min_r=min_range,max_r=max_range):
        ...
        x=self.place
        temp=0
        while(temp<min_r and x!=hive):
            temp+=1
            x=x.entrance
        while (x!=hive and temp<=max_r):
            if x.bees:
                return random.choice(x.bees)
            x=x.entrance
            temp+=1
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if(self.is_imposer==True):
            ...
            ScaryThrower.action(self,colony)
            p=self.place.exit
            while(p!=None):
                if(p.ant):
                    if(p.ant not in self.buff_ant):
                        p.ant.damage*=2
                        self.buff_ant.append(p.ant)
                    if(isinstance(p.ant,BodyguardAnt)):
                        if(p.ant.contained_ant and p.ant.contained_ant not in self.buff_ant):
                            p.ant.contained_ant.damage*=2
                            self.buff_ant.append(p.ant.contained_ant)
                p=p.exit
